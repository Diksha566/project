...
interface PlanDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertDayWorkout(workout: DayWorkoutEntity)

    @Query("SELECT * FROM day_workouts WHERE userId = :userId AND day = :day LIMIT 1")
    fun observeDayWorkoutEntity(userId: String, day: String): Flow<DayWorkoutEntity?>

    @Transaction
    @Query("SELECT * FROM day_workouts WHERE userId = :userId ORDER BY day ASC")
    fun observeWeeklyWorkouts(userId: String): Flow<List<DayWorkoutWithExercises>>

    @Transaction
    @Query("SELECT * FROM day_workouts WHERE userId = :userId AND day = :day LIMIT 1")
    fun observeWorkout(userId: String, day: String): Flow<DayWorkoutWithExercises?>
